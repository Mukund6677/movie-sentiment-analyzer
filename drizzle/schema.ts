...
export const analysisHistory = mysqlTable("analysis_history", {
  id: int("id").autoincrement().primaryKey(),
  ownerOpenId: varchar("ownerOpenId", { length: 64 }).notNull().default("anonymous"),
  datasetName: varchar("datasetName", { length: 255 }).notNull(),
  totalReviews: int("totalReviews").notNull(),
  accuracy: double("accuracy").notNull(),
  macroF1: double("macroF1").notNull(),
  positiveCount: int("positiveCount").notNull().default(0),
  neutralCount: int("neutralCount").notNull().default(0),
  negativeCount: int("negativeCount").notNull().default(0),
  matrixJson: text("matrixJson").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
